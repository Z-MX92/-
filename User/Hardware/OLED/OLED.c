...
/*简介与使用方法
OLED二分法逼近反相动画,需要与OLED_AnimUpdate配合使用,每调用一次反相
动画函数以后,动画只能执行一次,往后需要调用一次OLED_AnimUpdate,才可以接着再显示
一次动画,也就是每调用一次更新,就可以动一次,具体使用时可以每按下一次按钮进行一次
动画更新,就可以做到每按下按钮时动画,而不按下时不进行动画了,调用完该动画函数以后,
反相会自然停留在目标地点
##注意
*****************************************************
不需要再对目标地点进行反相,也不需要调用OLED_Update();
*****************************************************

  * #参数  X1,初始X位置,范围:0-127
  * #参数  Y1,初始Y位置,范围:0-63
  * #参数  L1,初始反相长度,范围:0-127
  * #参数  W1,初始反相高度,范围:0-63
  * #参数  X2,目标X位置,范围:0-127
  * #参数  Y2,目标X位置,范围:0-63
  * #参数  L2,目标反相长度,范围:0-127
  * #参数  W2,目标反相高度,范围:0-63
  * #返回值 无 
  */
